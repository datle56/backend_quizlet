...
class StudySet(Base):
    ...
    color = Column(String(20), nullable=True)  # Màu chủ đề

    # Relationships
    user = relationship("User", back_populates="study_sets")
    terms = relationship("Term", back_populates="study_set",
                         cascade="all, delete-orphan")
    versions = relationship(
        "StudySetVersion", back_populates="study_set", cascade="all, delete-orphan")
...
class Term(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    study_set_id = Column(Integer, ForeignKey(
        "study_sets.id"), nullable=False, index=True)
